            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                         ...
                         $sql6="SELECT * FROM borrow_db WHERE status_id=8 and DATEDIFF(date_return,Now())=1 "; 
                         $result6 = mysqli_query($conn,$sql6); 

     
                          ?>

                          <?php foreach ($result6 as $row5) { ?> 

                            <div class="card w-75 mb-3">
                            <div class="card-body">
                            <h5 class="card-title"> แจ้งเตือน</h5>
                            <p class="card-text">พรุ่งนี้ถึงกำหนดคืนเครื่อง <br> เลขที่ใบจอง : <?php  echo $row5['orderID']; ?> <br> วันครบกำหนด : <?php echo $row5['date_return']; ?></p>
                            <a  href="view_alert_tomorrow.php?id=<?=$row5['orderID']?>" class="btn btn-success mb-4" >เปิดดู</a>
                            </div>
                            </div>
                          ...
                          <?php } ?>
                           ...
                           <?php
                            $sql7="SELECT * FROM borrow_db WHERE status_id=8 and day(date_return)=day(now()) "; 
                            $result7 = mysqli_query($conn,$sql7);
                            ?>
                            <?php foreach ($result7 as $row6) { ?> 
                            <div class="card w-75 mb-3">
                            <div class="card-body">
                            <h5 class="card-title"> แจ้งเตือน</h5>
                            <p class="card-text">วันนี้ถึงกำหนดคืนเครื่อง <br> เลขที่ใบจอง : <?php  echo $row6['orderID']; ?> <br> วันครบกำหนด : <?php echo $row6['date_return']; ?></p>
                            <a  href="view_alert_today.php?id=<?=$row6['orderID']?>" class="btn btn-success mb-4" >เปิดดู</a>
                            </div>
                            </div>
                          <?php } ?>
                          ...
                          <?php
                             ...
                             $sql8="SELECT * FROM borrow_db WHERE status_id=7 ";
                             $result8 = mysqli_query($conn,$sql8);

                           ?>
                            <?php foreach ($result8 as $row7) { ?> 
                            <div class="card w-75 mb-3">
                            <div class="card-body">
                            <h5 class="card-title"> แจ้งเตือน</h5>
                            <p class="card-text">อนุมัติการจอง<br>เลขที่ใบจอง : <?php  echo $row7['orderID']; ?> <br> สามารถมารับเครื่องได้ </p>
                            <a  href="view_alert_approve.php?id=<?=$row7['orderID']?>" class="btn btn-success mb-4" >เปิดดู</a>
                            </div>
                            </div>
                          <?php } ?>
                           ...
                           <?php
                             ...
                             $sql9="SELECT * FROM borrow_db WHERE status_id=11 ";
                             $result9 = mysqli_query($conn,$sql9);

                             ?>
                              <?php foreach ($result9 as $row9) { ?> 
                                <div class="card w-75 mb-3">
                            <div class="card-body">
                            <h5 class="card-title"> แจ้งเตือน </h5>
                            <p class="card-text">ไม่อนุมัติการจอง<br>เลขที่ใบจอง : <?php  echo $row9['orderID']; ?> </p>
                            <a  href="view_alert_disapprove.php?id=<?=$row9['orderID']?>" class="btn btn-success mb-4" >เปิดดู</a>
                            </div>
                            </div>
                          <?php } ?>
                          ...
                          <?php
                            ...
                            $sql9="SELECT * FROM add_day_borrow WHERE status_id=7 ";
                            $result9 = mysqli_query($conn,$sql9);

                           ?>
                           <?php foreach ($result9 as $row8) { ?> 
                           <div class="card w-75 mb-3">
                           <div class="card-body">
                           <h5 class="card-title"> แจ้งเตือน</h5>
                           <p class="card-text">อนุมัติการยืมเครื่องต่อ<br>เลขที่ใบจอง : <?php  echo $row8['orderID']; ?> <br> จากวันที่ <?php echo $row8['date_return']; ?> <br> เป็นวันที่ <?php  echo $row8['new_date_return'] ; ?> </p>
                           <a  href="view_alert_addday.php?id=<?=$row8['orderID']?>" class="btn btn-success mb-4" >เปิดดู</a>
                           </div>
                           </div>
                           <?php } ?>
