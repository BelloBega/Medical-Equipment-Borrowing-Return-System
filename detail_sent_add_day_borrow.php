...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            and p.model_id=m.model_id and b.status_id='8' and r.status_id='8' and r.machinecode='$machinecode' and t.machinecode='$machinecode'and r.machinecode=t.machinecode
            and p.devicename_id=t.devicename_id and r.status_id=s.status_id
            order by o.pro_id and r.status_id "; 
            $result=mysqli_query($conn,$sql); 
            $row=mysqli_fetch_array($result);
            $date_return_new = date ("Y-m-d", strtotime("+3 day", strtotime($row['date_return'])));
             ?>
           <form method="POST" action="insert_add_day.php?id=<?=$row['orderID']?>&machinecode=<?=$row['machinecode']?>">
            ...
            <input type="text" name="machinecode" class="form-control" value="<?=$row['machinecode']?>" readonly > <br>  
            ...
            <label>วันที่เดิมที่ต้องคืนเครื่อง</label>
            <input type="text" name="date_return" class="form-control" value="<?=$row['date_return']?>" readonly><br>
            <label>วันที่ต้องคืนเครื่องใหม่ *สามารถยืมต่อได้สูงสุด 7 วัน เท่านั้น*</label>
            <input type="date" name="new_date_return" id="dateInput" class="form-control" min="<?php echo date ("Y-m-d", strtotime("+1 day", strtotime($row['date_return']))); ?>" max="<?php echo date ("Y-m-d", strtotime("+7 day", strtotime($row['date_return']))); ?>"  > <br> 
            <label>เหตุผลในการขอยืมเครื่องต่อ</label>
            <input type="text" name="reason" class="form-control" > <br>
            <input type="submit" value="บันทึก" class="btn btn-success">
            <a href="sent_add_day_borrow.php?id=<?=$row['orderID']?>" class="btn btn-danger mb-4 mt-4">ยกเลิก</a>
               </div>
        </form>
        </div>
     </div>
