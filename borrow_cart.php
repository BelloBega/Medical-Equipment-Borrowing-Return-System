...
<body id="page-top">
    ...
    <div id="wrapper">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">จองเครื่อง</h1>
                    </div>

                    <!-- Content Row -->
                    <div class="row">

                    <form id="form1" method="POST" action="insert_borrow.php">
             <div class="row">
                <div class="col-md-10">
        ...
        <?php
              $Total = 0; // จำนวน
              $m = 1; // การเพิ่มลดทีละค่า
                   for ($i=0; $i <= (int)$_SESSION["intLine"]; $i++){
                      if(($_SESSION["strProductID"][$i])!=""){
      
                        $sql1="SELECT * 
                        ,( select count(*) from product where model_id = p.pro_id) as count_product
                        ,( select count(*) from product where model_id = p.pro_id) as amount
                        FROM product_borrow p , product_devicename d , product_brand b , product_model m, status_tbl s 
                        WHERE p.status_id=s.status_id and p.devicename_id = d.devicename_id and p.brand_id = b.brand_id 
                        and p.model_id=m.model_id and p.pro_id='" . $_SESSION["strProductID"][$i]. "' 
              ...
              ?>              
                        <tr>
                            <td><?=$m?></td>
                            <td><img src="images/<?=$row_pro['images']?>" width="80" height="100" ><?= $row_pro['devicename']?></td>
                            <td><?= $row_pro['brand_name']?></td>
                            <td><?= $row_pro['model_name']?></td>
                            <td><?= $_SESSION["strQty"][$i]?></td>
                            <td>
                                <a href="borrow_order.php?id=<?=$row_pro['pro_id']?>" class="btn btn-outline-primary ">+</a>
                                <?php if($_SESSION["strQty"][$i] > 1 ){ ?>
                                <a href="borrow_order_del.php?id=<?=$row_pro['pro_id']?>" class="btn btn-outline-primary ">-</a>
                                <?php } ?>
                            </td>
                            <td>
                                <a href = "borrow_delete.php?Line=<?=$i?>"><img src="images/delete.png" width="30"></a>
                            </td>
                        </tr>
        <?php
            $m=$m+1;
        ...
        ?>
                    ...
                    <div style="text-align:right">
                               <a href ="user_page.php"><button type="button" class="btn btn-outline-secondary">เลือกเครื่องมือแพทย์</button></a>
                               <button type="submit" class="btn btn-outline-primary">ยืนยันการจอง</button>
                            </div>
                            </div>
                      <br>

                      <?php 
                            $sql8="SELECT * FROM register_db ";
                            $result8=mysqli_query($conn,$sql8);
                            $row_re = mysqli_fetch_array($result8);
                ...
                <div align="center" >
                   <div style="text-align:left">
                    <div class="col col-sm-8">
                        <div class="h1 text-center alert alert-info mb-4 mt-4" role="alert">กรุณากรอกข้อมูลสำหรับการขอจองเครื่องมือแพทย์</div>
                        <label>รหัสผู้ใช้งาน</label>
                        <input type="text" name="m_id" class="form-control"  value="<?php echo $_SESSION['m_id']; ?>" readonly ><br>
                        <label>ชื่อผู้ขอจอง</label>
                        <input type="text" name="borrower" class="form-control" value="<?php echo $_SESSION['user']; ?>" readonly><br>
                        <label>วันที่จะยืม</label>
                        <input type="date" name="date_start" class="form-control" id="start" min="<?php echo date('Y-m-d')?>" max="<?php echo date("Y-m-d",strtotime("+31day")) ?>"><br>
                        <label>วันที่จะคืน</label>
                        <input type="date" name="date_return" class="form-control" id="start" min="<?php echo date('Y-m-d')?>" max="<?php echo date("Y-m-d",strtotime("+31day")) ?>"><br>
                    </div>
                  </div>
              </div>
            </form>
