...
include 'condb.php'; 
 $ids=$_GET['id'];  
 $sql="SELECT * FROM borrow_db b , register_db r ,status_tbl s  WHERE b.status_id IN (6,7,11) and b.m_id=r.m_id "; 
...
<body id="page-top">
 ...
 <?php 
             ...
             WHERE o.orderID=b.orderID  and o.pro_id=p.pro_id and o.orderID='$ids' and b.status_id IN (6,7,11) and p.devicename_id=d.devicename_id and p.brand_id=n.brand_id 
 ...
 ?> 
             ...
             <?php foreach ($result as $row) { ?> 
                ...
                <td> 
                <a href="approve_borrow_detail.php?id=<?=$row['orderID']?>&pro_id=<?=$row['pro_id']?>" class="btn btn-success mb-4">อนุมัติ</a> 
                </td> 
             </tr> 
         </tbody>  
              ...
              ?>     
               ...
               <a href="approve_borrow.php?status_id=<?=$row['status_id']?>&id=<?=$row['orderID']?>&" class="btn btn-warning mb-4 mt-4" onclick="Del1(this.href);return false;">อนุมัติทั้งหมด</a> 
               <a href="disapprove_borrow.php?status_id=<?=$row['status_id']?>&id=<?=$row['orderID']?>&" class="btn btn-danger mb-4 mt-4" onclick="Del(this.href);return false;">ไม่อนุมัติทั้งหมด</a> 
               <a href="request_borrow_db.php" class="btn btn-success mb-4 mt-4">เสร็จสิ้น</a> 
          </div> 
          </div> 
...
function Del1(mypage1){ 
     var agree=confirm("อนุมัติการจองทั้งหมด"); 
     if(agree){ 
       window,location=mypage1; 
     } 
   } 
  
   function Del(mypage1){ 
     var agree=confirm("ไม่อนุมัติการจองทั้งหมด"); 
     if(agree){ 
       window,location=mypage1; 
     } 
   } 
